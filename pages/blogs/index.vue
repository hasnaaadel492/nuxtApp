...
watchEffect(() => {
  ...
  if (lang.value === "en") {
    nextTick(() => {
      const blogsComponent = document.querySelector(".blogsComponent");

      if (!blogsComponent) {
        console.warn("`.blogsComponent` not found in the DOM.");
        return;
      }
      ...
      selectors.forEach((selector) => {
        const elements = blogsComponent.querySelectorAll(selector);
